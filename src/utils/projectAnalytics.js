...
export function calculateCriticalPath(tasks, dependencies) {
  if (!tasks.length) return { ids: [], totalDays: 0, names: [] };
  ...
  const indegree = new Map(
    [...incoming.entries()].map(([id, preds]) => [id, preds.length])
  );
...
export function getProjectHealth({ overdueCount, riskyCount, upcomingCount }) {
  if (overdueCount > 0 || riskyCount > 0) {
    return { label: 'Риск', tone: 'danger', text: 'Есть задачи, требующие внимания.' };
  }
  if (upcomingCount > 0) {
    return { label: 'Внимание', tone: 'warning', text: 'Скоро наступают сроки задач.' };
  }
  return { label: 'В норме', tone: 'success', text: 'Критичных проблем по срокам не найдено.' };
}
