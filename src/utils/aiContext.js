import { durationDays, effectiveStatus, isTaskOverdue, overdueDays } from './projectAnalytics';
...
export function buildAIProjectContext({
  ...
  for (const dependency of dependencies) {
    ...
    if (!predecessorsByTask.has(successorId)) predecessorsByTask.set(successorId, []);
    predecessorsByTask.get(successorId).push(taskNamesById.get(predecessorId) || `Задача #${predecessorId}`);
  }
  ...
  return {
    ...
    tasks: tasks.map((task) => ({
      ...
      overdue_days: overdueDays(task),
      progress: Number(task.progress || 0),
      comments: task.comments || '',
      assignee: task.assignee_id == null ? null : usersById.get(String(task.assignee_id)) || `ID ${task.assignee_id}`,
