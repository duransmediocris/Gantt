...
function App() {
  ...
  // Список пользователей с backend
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      // Не ломаем всё приложение,
      // если backend ещё не обновили.
      setUsers([]);
    }
  }, []);
  ...
  const handleTaskDateChange = async (ganttTask) => {
    try {
      ...
      alert(
        `Не удалось изменить даты: ${err.message}`
      );
  ...
  const handleTaskProgressChange = async (ganttTask) => {
    try {
      ...
      alert(
        `Не удалось изменить прогресс: ${err.message}`
      );
  ...
  const handleSaveTask = async (formData) => {
    ...
    // Backend при POST задачи сохраняет
    // основные поля.
    // Статус и прогресс обновляем отдельно.
    if (
      payload.status !== 'todo' ||
      payload.progress !== 0
    ) {
      ...
      <div style={styles.container}>
        <div style={styles.header}>
          <div>
            ...
            <div style={styles.subtle}>
              Frontend подключён к Render backend
            </div>
          </div>
        ...
        {!loading && project && (
          <>
            ...
            >
              <b>Подсказка:</b>{' '}
              перетаскивай задачи на диаграмме —
              backend пересчитает каскадный сдвиг
              зависимых задач. Двойной клик по
              задаче открывает редактирование.
            </div>
          </>
        )}
      </div>
